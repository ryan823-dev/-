  ...
  return (
    <header className="h-16 bg-[#FFFCF6] border-b border-[#E7E0D3] px-6 flex items-center justify-between shrink-0 z-20">
      ...
      <div className="flex items-center gap-1.5 bg-[#F7F3EA] px-5 py-2 rounded-full border border-[#E7E0D3] text-sm">
        ...
        <span className="text-[10px] bg-[#C7A56A]/10 text-[#C7A56A] px-1.5 py-0.5 rounded font-mono border border-[#C7A56A]/20">
        ...
        <div className="relative" ref={dropdownRef}>
          <button 
            ...
            className={`flex items-center gap-2.5 pl-2 pr-4 py-1.5 rounded-full border-2 transition-all select-none active:scale-[0.98] ${
              isDecider
                ? 'bg-[#0B1B2B] border-[#C7A56A]/40 hover:border-[#C7A56A]/70'
                : 'bg-[#0B1B2B] border-blue-400/30 hover:border-blue-400/60'
            }`}
          >
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-[10px] font-bold ${
              isDecider ? 'bg-[#C7A56A] text-[#0B1B2B]' : 'bg-blue-500 text-white'
            }`}>
              {roleLabel.slice(0, 1)}
            </div>
            <div className="text-left hidden sm:block">
              ...
              <div className="flex items-center gap-1 mt-1">
                <Shield size={9} className={isDecider ? 'text-[#C7A56A]' : 'text-blue-400'} />
          ...
          {isDropdownOpen && (
            <div className="absolute right-0 mt-2 w-64 bg-white border border-[#E7E0D3] rounded-2xl shadow-xl overflow-hidden z-50 animate-fade-in">
              ...
              <div className="p-3">
                {/* 决策者 */}
                <div className={`flex items-center gap-3 p-3 rounded-xl ${isDecider ? 'bg-[#F7F3EA] border border-[#C7A56A]/20' : 'opacity-50'}`}>
                  <div className={`w-9 h-9 rounded-lg flex items-center justify-center text-xs font-bold ${isDecider ? 'bg-[#C7A56A] text-[#0B1B2B]' : 'bg-[#C7A56A]/30 text-[#C7A56A]'}`}>
                    决
                  </div>
                  <div>
                  ...
                  {isDecider && <Shield size={14} className="text-[#C7A56A] ml-auto" />}
                </div>
