...
输出严格JSON格式：
{
  ...
  "openQuestions": ["待确认问题汇总"],
  "confidence": 0.85
}

规则：
1. segmentation 是单一对象(不是数组),包含完整的firmographic/technographic/triggers/decisionUnit
2. 规则必须可落地执行
3. 不编造事实，标注假设
4. confidence 是置信度(0-1之间)
只输出JSON。`;
...
规则：
1. 至少包含5种渠道(maps/tender/search/directory + 1种以上)
2. 每个渠道必须给出可执行的searchQueries和signalsToLookFor
3. priority是优先级(1最高)
4. confidence是置信度(0-1之间)
只输出JSON。`;
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log('[radar-sync] TargetingSpec generated:', JSON.stringify(targetingParsed, null, 2).slice(0, 500) + '...');
    ...
    console.log('[radar-sync] ChannelMap generated:', JSON.stringify(channelParsed, null, 2).slice(0, 500) + '...');
